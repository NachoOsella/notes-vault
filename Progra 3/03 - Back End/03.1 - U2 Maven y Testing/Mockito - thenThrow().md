...
- vs [[Mockito - thenReturn()]]: `thenThrow` simula fallas/excepciones; `thenReturn` simula respuestas normales/exitosas.
- vs [[Mockito - doNothing()]]: para métodos `void`, en vez de `thenThrow` se usa `doThrow().when(mock).metodo()`.
