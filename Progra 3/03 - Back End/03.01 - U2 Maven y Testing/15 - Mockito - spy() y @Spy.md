...
- vs [[14 - Mockito - mock() y @Mock]]: spy mantiene comportamiento real por default; mock no tiene comportamiento (devuelve null/0/false).
- vs [[17 - Mockito - when()]]: con spies, `when` puede ejecutar el método real (cuidado); a veces conviene usar `doReturn`/`doThrow`.
