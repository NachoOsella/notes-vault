...
- vs [[API - Qué es una API]]: todo servicio web es una API; no toda API es servicio web (puede ser local o no usar HTTP).
- vs [[HTTP - Request]] / [[HTTP - Response]]: un servicio web se consume via request/response sobre HTTP/HTTPS.
- vs [[API - REST vs SOAP]]: REST y SOAP son estilos/protocolos comunes para implementar servicios web.
