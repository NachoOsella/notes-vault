...
- vs [[HTTP - Response]]: el status code es el indicador de resultado dentro de la response completa.
- vs [[HTTP - Request]]: el request no tiene códigos de estado; los códigos aparecen en la respuesta del servidor.
- vs [[HTTP - Métodos de solicitud]]: método = qué querés hacer; status = qué pasó cuando se intentó.
