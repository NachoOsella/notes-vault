...
- vs [[HTTP - Response]]: request es la solicitud (qué querés y cómo); response es el resultado (qué pasó y qué te devuelven).
- vs [[HTTP - Métodos de solicitud]]: el método (GET/POST/...) es solo una parte del request; el request incluye también URL, headers y body.
- vs [[HTTP - Flujo de mensajes]]: el request es un mensaje; el flujo describe cuándo y cómo se envía dentro de la interacción.
