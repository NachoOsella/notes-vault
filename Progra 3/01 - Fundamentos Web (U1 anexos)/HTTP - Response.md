...
- vs [[HTTP - Request]]: la response es la contestación del servidor a un request (misma interacción, distinto sentido).
- vs [[HTTP - Códigos de estado]]: el status code es una parte de la response; la response además incluye headers y body.
- vs [[HTTP - Flujo de mensajes]]: response es el mensaje de vuelta; el flujo describe el proceso completo (conexión, ida y vuelta).
