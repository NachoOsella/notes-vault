...
- vs [[API - Qué es un Servicio Web]]: servicio web es una API expuesta por red (normalmente HTTP); una API puede ser local (sin red).
- vs [[API - Tipos de APIs (locales vs remotas vs web)]]: acá se define qué es una API; el otro clasifica APIs según dónde y cómo corren.
- vs [[API - Arquitectura RESTful (API REST)]]: REST es una forma común de implementar servicios web (un tipo de API), no la definición de API en sí.
